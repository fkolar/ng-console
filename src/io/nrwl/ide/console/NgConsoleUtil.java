...
public class NgConsoleUtil {
    ...
    public static void startServer() {
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
            ...
            try {
                NgConsoleUtil.ourServer = new NgConsoleServer();

            } catch (Exception e) {
                LOG.error("Problem while starting server");
            }
        });

    }

    public static void stopServer(boolean async) {
        Runnable action = () -> NgConsoleUtil.ourServer.shutdown();
    ...
    public static boolean isAngularProject(@NotNull Project project, @Nullable VirtualFile file) {
